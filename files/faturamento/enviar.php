...
switch ($_POST['assunto']) {
    case '0':
        ...
        break;
    case '1':
       $assunto = "Agendamento de espaço";
        break;
    case '2':
       $assunto = "Reserva de equipamento";
        break;
...
if(isset($_POST['equipamento1'])){
    $show = $_POST['equipamento1'];
    $body = str_replace('$show', $show, $body);

}
if(isset($_POST['equipamento2'])){
    $notebook = $_POST['equipamento2'];
    $body = str_replace('$notebook', $notebook, $body);
}
if(isset($_POST['equipamento3'])){
    $caixa = $_POST['equipamento3'];
    $body = str_replace('$caixa', $caixa, $body);
}
if(isset($_POST['equipamento4'])){
    $microfone = $_POST['equipamento4'];
    $body = str_replace('$microfone', $microfone, $body);
}

if(isset($_POST['inicial'])){
    $inicial = date("j/n/Y",strtotime($_POST['inicial']));
    $body = str_replace('$inicial', $inicial, $body);
}
if(isset($_POST['final'])){
    $final= date("j/n/Y",strtotime($_POST['final']));
    $body = str_replace('$final', $final, $body);
}else{
    $body = str_replace('$final', $inicial, $body);
}
...
if(!$mail->Send()) {
  echo "Mailer Error: " . $mail->ErrorInfo;
    ...
    echo "<script>alert('Sua solicita\u00e7\u00e3o foi envida com sucesso!');</script>";
